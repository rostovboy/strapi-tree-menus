...
const console__namespace = /* @__PURE__ */ _interopNamespace(console);
...
const getService = (name, { strapi: strapi2 } = { strapi: global.strapi }) => {
  return strapi2.plugin(PLUGIN_ID).service(name);
};
const register = ({ strapi: strapi2 }) => {
  ...
  if (strapi2.plugin("documentation")) {
    ...
    strapi2.plugin("documentation").service("override").registerOverride(overrides, {
...
const requestDataYupSchema = () => {
  ...
  return yup__namespace.object().shape({
    title: yup__namespace.string().max(100).required(),
    slug: yup__namespace.string().required(),
    items: yup__namespace.lazy(() => itemSchema)
  });
};
...
const menu$3 = strapi.factories.createCoreController(UID_MENU, ({ strapi: strapi2 }) => ({
  ...
  async find(ctx) {
    ...
    const limit = lodash.get(query, "pageSize", 10);
    const start = lodash.get(query, "page", 1) * limit - limit;
    ctx.request.query = lodash.omit(query, ["page", "pageSize"]);
  ...
  async deleteMany(ctx) {
    ...
    const { data } = body || {};
    const ids = data.ids;
    const locale = lodash.get("locale", query) || lodash.get("locale", body.data) || void 0;
...
const menu = strapi.factories.createCoreService("plugin::tree-menus.menu", ({ strapi: strapi2 }) => ({
  /**
   * Find all menus
   * @param ctx
   */
  ...
  async deleteMany(ids, locale) {
    return await strapi2.db.transaction(async ({ rollback, commit }) => {
      try {
        for (const id of ids) {
        ...
        console__namespace.error("menu service deleteMany", e);
        await rollback();
      }
    });
  }
...
const documentation = ({ strapi: strapi2 }) => ({
...
const config = ({ strapi: strapi2 }) => ({
  get() {
    return strapi2.config.get(`plugin.${pluginId}`, config$1);
  },
  async schema() {
    const menuModel = strapi2.getModel(UID_MENU);
    return {
